...
struct GamepadState
{
	...
	bool wasButtonReleased(int num) { return (!button[num] & prev_button[num]) != 0; }
...
public:
	...
	//keyboard
	static bool isKeyPressed(int key_code, bool force = false) { return  isImguiFocused() && !force ? 0 : keystate[key_code] != 0; }
	static bool wasKeyPressed(int key_code, bool force = false) { return isImguiFocused() && !force ? 0 : keystate[key_code] != 0  && prev_keystate[key_code] == 0; }
	static bool wasKeyReleased(int key_code, bool force = false) { return isImguiFocused() && !force ? 0 : keystate[key_code] == 0 && prev_keystate[key_code] != 0; }

	//gamepad state
	static GamepadState gamepads[4];

	//gamepad
	static bool isButtonPressed(int button, int pad = 0, bool force = false) { return  isImguiFocused() && !force ? 0 : gamepads[pad].isButtonPressed(button); }
	static bool wasButtonPressed(int button, int pad = 0, bool force = false) { return isImguiFocused() && !force ? 0 : gamepads[pad].wasButtonPressed(button); }
	static bool wasButtonReleased(int button, int pad = 0, bool force = false) { return isImguiFocused() && !force ? 0 : gamepads[pad].wasButtonReleased(button); }

	//mouse
	static bool isMousePressed(int button, bool force = false) { return isImguiFocused() && !force ? 0 : mouse_state & SDL_BUTTON(button); } //button could be SDL_BUTTON_LEFT
