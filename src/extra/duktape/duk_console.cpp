...
static duk_ret_t duk__console_log_helper(duk_context *ctx, const char *error_name) {
	...
	if(output == NULL)
		output = (flags & DUK_CONSOLE_STDOUT_ONLY) ? stdout : stderr;
	...
	if (error_name) {
		...
		outstring = duk_to_string(ctx, -1);
		if (std::string(error_name).compare("Error") == 0)
			Script::Console::items.push_back("[error] " + std::regex_replace(outstring, std::regex("Error: "), ""));
		else if (std::string(error_name).compare("Warn") == 0)
			Script::Console::items.push_back("[warn] " + std::regex_replace(outstring, std::regex("Warn: "), ""));
	...
	{
		outstring = duk_to_string(ctx, -1);
		Script::Console::items.push_back(outstring);
	}

	fprintf(output, "%s\n", outstring.c_str());
	//Script::Console::items.push_back()
...
void duk_console_init_WF(duk_context* ctx, duk_uint_t flags, FILE* out) {
	output = out;
