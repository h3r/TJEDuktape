...
HBTGraph.prototype._ctor = function( name )
{
	...
	this.uid = performance.now().toString();//generateUId('HBT'); 
...
// {
// 	if(this.graph.context.facade.entityInTarget(agent, this.facade.getEntityPropertyValue("target", agent), this.properties.threshold))
// 	{
// 		// check if the target is in some special list and  some properties to apply to
// 		// the agent or to the blackboard
// 		if(CORE && CORE.Scene)
// 		{
// 			CORE.Scene.applyTargetProperties(this.facade.getEntityPropertyValue("target", agent), agent);
// 			return true;
// 		}
// 	}
// 	else
...
RandomSelector.prototype.tick = function(agent, dt)
{
	if(agent.bt_info.running_node_index != null && agent.bt_info.running_node_id == this.id)
	{
		if(agent.random_period != null && agent.random_period != undefined)
			agent.random_period += dt;
		...
		if(value.STATUS == STATUS.success )
		{
			//ahagagagajagahjgahjaghajahgajgshjahjajhs
			agent.bt_info.running_node_index = null;
			agent.bt_info.running_node_id = null;
			if(agent.is_selected)
			{
	...
	// no running node in its children
	else
	{
		if(agent.random_period == undefined || agent.random_period == null)
			agent.random_period = 0;
		else
			agent.random_period += dt;
		...
		if( agent.random_period <= 0 || agent.random_period > this.properties.period )
		{
			//reset agent.random_period && rand
			agent.random_period = 0;
			//random child to tick
			var rand = Math.trunc(Math.random()*children.length);
			agent.rand_selection_index = rand;
		}

		//if we are still inside the random_period time, execute again the same rand node

		var child_to_tick = children[agent.rand_selection_index];
		var value = child_to_tick.tick(agent, dt);
		...
		if(value.STATUS == STATUS.running)
		{
			...
			else
			{
				...
				agent.bt_info.running_node_index = parseInt(agent.rand_selection_index);
...
LookAt.prototype.tick = function(agent, dt)
{
	...
	this.facade.setEntityProperty(agent, "look_at_pos", this.properties.look_at.pos );
...
function PoseWeighter()
{
	...
	this.addOutput("dist","number", {pos:[120,15], dir:LiteGraph.LEFT});
	    this.size = [120,30];
    this.editable = { property:"value", type:"number" };
  	this.widgets_up = true;
	// this.horizontal = true;
